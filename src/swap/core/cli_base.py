...
def cmd_status(args, ctx):
    ...
    if slot:
        ...
        print(
            f"Active: slot {slot} — {_account_label(acc)} "
...
def build_parser(provider: Provider) -> argparse.ArgumentParser:
    ...
    sp_seed.add_argument(
        "targets",
        nargs="*",
        help="slot numbers / emails (default: only slots without data)",
    )
    sp_seed.add_argument(
        "--all", action="store_true", help="Re-seed every slot, even those with data"
    )
    sp_seed.add_argument(
        "--concurrency", type=int, default=4, help="Max parallel probes (default: 4)"
    )
    sp_seed.add_argument(
