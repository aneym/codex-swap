...
Verbatim port of `codex_swap.usage`, with the persisted store path and the
sequence path resolved from `swap.core.paths` so multiple providers don't share
state.
"""
...
def _logs_db() -> Path:
    return _codex_home() / "logs_2.sqlite"
...
def scan_rollouts_for_usage() -> dict[str, dict]:
    seq = read_json(_sequence_path()) or {}
    accounts = seq.get("accounts", {}) or {}
    if not accounts:
        return {}

    by_account = {
        acc.get("account_id"): slot
