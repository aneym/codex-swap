"""Claude provider adapter — binds the Claude Code surface to the core protocol."""
...
def _swap_root():
    return provider_root("claude")


CLAUDE = Provider(
    name="claude",
    display_name="Claude",
    cli_prog="claude-swap",
    cli_description="Multi-account switcher for the Claude Code CLI.",
    swap_root=_swap_root,
    # credentials
    read_live_credentials=_auth.read_live_credentials,
    clear_live_credentials=_auth.clear_live_credentials,
    credentials_identity=_auth.credentials_identity,
    credentials_fingerprint=_auth.credentials_fingerprint,
    # slot snapshot
    snapshot_to_slot=_auth.snapshot_to_slot,
    restore_from_slot=_auth.restore_from_slot,
    # oauth / login
    start_login=_oauth.start_login,
    # probes (stubs in v0.1)
    ...
    onboard_label="Claude accounts",
    quiet_env_var="CLAUDE_SWAP_QUIET",
)
